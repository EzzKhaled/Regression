...
def run_regression():
    ...
    # REQUIRED: Plot validation error vs regularization parameter
    print("\n" + "="*50)
    print("3. VALIDATION ERROR VS REGULARIZATION PARAMETER PLOTS")
    print("="*50)
    plot_validation_error_vs_regularization(manual_results, sklearn_results)
    
    # Additional recommended plots
    print("\n" + "="*50)
    print("4. ADDITIONAL DIAGNOSTIC PLOTS")
    print("="*50)
    plot_predicted_vs_actual(manual_results, sklearn_results, X_test_bias, y_test, feature_names)
    plot_residuals(manual_results, sklearn_results, X_test_bias, y_test, feature_names)
...
def plot_validation_error_vs_regularization(manual_results, sklearn_results):
    """Plot validation error vs regularization parameter - REQUIRED PLOT"""
    print("\n--- Generating Validation Error vs Regularization Parameter Plots ---")
    manual_alphas = [0.001, 0.01, 0.1, 1, 10, 100]
    sklearn_alphas = [0.001, 0.01, 0.1, 1, 10, 100]
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
    ax1.plot(manual_alphas, manual_results['ridge']['val_errors'], 'bo-', 
             linewidth=2, markersize=6, label='Validation MSE')
    ax1.axvline(x=manual_results['ridge']['best_alpha'], color='red', linestyle='--', 
                label=f'Best α = {manual_results["ridge"]["best_alpha"]}', alpha=0.7)
    ax1.set_xscale('log')
    ax1.set_xlabel('Regularization Parameter (alpha)')
    ax1.set_ylabel('Validation MSE')
    ax1.set_title('Manual Ridge Regression\nValidation Error vs Regularization Parameter')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    # Manual Lasso Regression
    ax2.plot(manual_alphas, manual_results['lasso']['val_errors'], 'ro-', 
             linewidth=2, markersize=6, label='Validation MSE')
    ax2.axvline(x=manual_results['lasso']['best_alpha'], color='blue', linestyle='--', 
                label=f'Best α = {manual_results["lasso"]["best_alpha"]}', alpha=0.7)
    ax2.set_xscale('log')
    ax2.set_xlabel('Regularization Parameter (alpha)')
    ax2.set_ylabel('Validation MSE')
    ax2.set_title('Manual Lasso Regression\nValidation Error vs Regularization Parameter')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    # Scikit-Learn Ridge Regression
    ax3.plot(sklearn_alphas, sklearn_results['ridge']['val_errors'], 'bo-', 
             linewidth=2, markersize=6, label='Validation MSE')
    ax3.axvline(x=sklearn_results['ridge']['best_alpha'], color='red', linestyle='--', 
                label=f'Best α = {sklearn_results["ridge"]["best_alpha"]}', alpha=0.7)
    ax3.set_xscale('log')
    ax3.set_xlabel('Regularization Parameter (alpha)')
    ax3.set_ylabel('Validation MSE')
    ax3.set_title('Scikit-Learn Ridge Regression\nValidation Error vs Regularization Parameter')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    
    # Scikit-Learn Lasso Regression
    ax4.plot(sklearn_alphas, sklearn_results['lasso']['val_errors'], 'ro-', 
             linewidth=2, markersize=6, label='Validation MSE')
    ax4.axvline(x=sklearn_results['lasso']['best_alpha'], color='blue', linestyle='--', 
                label=f'Best α = {sklearn_results["lasso"]["best_alpha"]}', alpha=0.7)
    ax4.set_xscale('log')
    ax4.set_xlabel('Regularization Parameter (alpha)')
    ax4.set_ylabel('Validation MSE')
    ax4.set_title('Scikit-Learn Lasso Regression\nValidation Error vs Regularization Parameter')
    ax4.legend()
...
def plot_predicted_vs_actual(manual_results, sklearn_results, X_test, y_test, feature_names):
    """Plot predicted vs actual values for the best model"""
    print("\n--- Generating Predicted vs Actual Values Plot ---")
    

    best_manual_model = manual_results['ridge']['model']
    best_sklearn_model = sklearn_results['ridge']['model']
    
    # Get predictions
    if hasattr(X_test, 'shape') and len(X_test.shape) == 2 and X_test.shape[1] == len(feature_names) + 1:
        manual_pred = best_manual_model.predict(X_test)
        sklearn_pred = best_sklearn_model.predict(X_test[:, 1:])  
    else:
        manual_pred = best_manual_model.predict(X_test)
        sklearn_pred = best_sklearn_model.predict(X_test)
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    
    # Manual predictions
    ax1.scatter(y_test, manual_pred, alpha=0.5, color='blue')
    ...
    ax2.scatter(y_test, sklearn_pred, alpha=0.5, color='red')
    ax2.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--', lw=2)
    ax2.set_xlabel('Actual House Value ($)')
    ax2.set_ylabel('Predicted House Value ($)')
    ax2.set_title('Scikit-Learn Ridge Regression\nPredicted vs Actual Values')
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.show()

def plot_residuals(manual_results, sklearn_results, X_test, y_test, feature_names):
    ...
    manual_residuals = y_test - manual_pred
    sklearn_residuals = y_test - sklearn_pred
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    
    # Manual residuals
    ax1.scatter(manual_pred, manual_residuals, alpha=0.5, color='blue')
    ax1.axhline(y=0, color='r', linestyle='--')
    ax1.set_xlabel('Predicted House Value ($)')
    ax1.set_ylabel('Residuals ($)')
    ax1.set_title('Manual Ridge Regression\nResidual Plot')
    ax1.grid(True, alpha=0.3)
    
    # Scikit-learn residuals
    ax2.scatter(sklearn_pred, sklearn_residuals, alpha=0.5, color='red')
